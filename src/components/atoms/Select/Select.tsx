...
export function Select<T, U extends ReactNode, V extends ReactNodeOrFunctionType>(props: SelectProps<T, U, V>) {
  ...
  const { optionIndex, optionSelected, optionsWithKey } = useMemo(() => {
    ...
    initialOptionSelected.value,
    initialOptionSelected.label,
    initialOptionSelected.inputLabel,
    initialOptionSelected.key,
    options,
  ]);
  ...
  return (
    <Popover
      ...
      popoverClassName={classNames('jk-select-options-content bc-sf-hi jk-br-ie elevation-1', popoverClassName)}
      offset={4}
      open={isOpen}
    ...
    >
      ...
      >
        ...
        >
          {children ? (
            renderReactNodeOrFunctionP1(children, { options, isOpen, disabled: isDisabled, optionSelected, expandIcon })
          ) : (
