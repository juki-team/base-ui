...
export const arcS = (
  { x: p1x, y: p1y }: Vector | { x: number; y: number },
  { x: p2x, y: p2y }: Vector | { x: number; y: number },
  k = 0.5,
) => {
...
export const M = ({ x, y }: { x: number; y: number } | Vector) => {
  // move to point without a draw
  return `M ${x},${y}`;
};

export const H = (x: number) => {
  // draw a line
  return `H ${x}`;
};

export const V = (y: number) => {
  // draw a line
  return `V ${y}`;
};

export const L = ({ x, y }: { x: number; y: number } | Vector) => {
  // draw a line
  return `L ${x},${y}`;
};

export const cLB = () => {
  // Curve left bottom // svg: left top
  return 'c -0.5,0  -1,-0.5 -1,-1';
};

export const cBR = () => {
  // Curve bottom right // svg: top right
  return 'c 0,-0.5 0.5,-1 1,-1';
};

export const cRT = () => {
  // Curve right top // svg: right bottom
  return 'c 0.5,0 1,0.5 1,1';
};

export const cTL = () => {
  // Curve top left // svg: bottom left
  return 'c 0,0.5 -0.5,1 -1,1';
};
