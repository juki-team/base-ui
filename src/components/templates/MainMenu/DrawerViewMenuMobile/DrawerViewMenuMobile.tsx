import { Children, type ReactNode, useState } from 'react';
import { useUIStore } from '../../../../stores/ui/useUIStore';
import { useUserStore } from '../../../../stores/user/useUserStore';
import { Div, T } from '../../../atoms';
import { classNames } from '../../../helpers/commons';
import { renderReactNodeOrFunction, renderReactNodeOrFunctionP1 } from '../../../helpers/render';
import type { MenuType } from '../../../organisms/types';
import { ArrowBackIcon, CloseIcon } from '../../../server';
import { HelpSection } from '../../HelpSection/HelpSection';
import { SettingsSection } from '../SettingsSection';

export interface DrawerViewMenuMobileProps {
  onClose: () => void;
  menu: MenuType[];
  logoImageUrl: string;
  moreApps?: ReactNode;
}

export const DrawerViewMenuMobile = ({ onClose, menu, logoImageUrl, moreApps }: DrawerViewMenuMobileProps) => {
  const { Image } = useUIStore((store) => store.components);
  const companyName = useUserStore((state) => state.organization.name);
  const [helpOpen, setHelpOpen] = useState(false);

  return (
    <div className="jk-drawer-menu-mobile jk-col top nowrap stretch" style={{ backgroundColor: 'var(--cr-we-dark)' }}>
      <div
        className="jk-row extend center nowrap bc-sf-md elevation-1 pn-re"
        style={{ height: 'var(--top-horizontal-menu-height)' }}
      >
        <div className="jk-row jk-pg-rl" style={{ position: helpOpen ? undefined : 'absolute', left: 0 }}>
          {helpOpen ? (
            <ArrowBackIcon className="clickable jk-br-ie" onClick={() => setHelpOpen(false)} />
          ) : (
            <CloseIcon className="clickable jk-br-ie" onClick={onClose} />
          )}
        </div>
        <div className="jk-row extend">
          {helpOpen ? (
            <div className="jk-row extend left">
              <T className="tt-se">help</T>
            </div>
          ) : (
            <Image src={logoImageUrl} alt={companyName} height={45} width={90} />
          )}
        </div>
      </div>
      {helpOpen ? (
        <div className="jk-col gap top jk-pg-rl jk-pg-tb">
          <HelpSection />
        </div>
      ) : (
        <>
          <div className="jk-menu-items jk-col gap stretch top jk-pg-rl jk-pg-tb flex-1">
            {Children.toArray(
              menu.map(({ selected, icon, label, menuItemWrapper }, index) => {
                const menuItem = (
                  <Div
                    // biome-ignore lint/suspicious/noArrayIndexKey: menu is a fixed list per render; Children.toArray wraps with stable keys
                    key={index}
                    className={classNames('jk-menu-item jk-row extend gap nowrap left jk-br-ie bc-pd cr-tx-ht-it', {
                      selected: !!selected,
                    })}
                    onClick={onClose}
                    onKeyDownClick
                  >
                    {icon && <div>{renderReactNodeOrFunction(icon)}</div>}
                    <div className="jk-menu-item-label">{renderReactNodeOrFunction(label)}</div>
                  </Div>
                );

                if (menuItemWrapper) {
                  return renderReactNodeOrFunctionP1(menuItemWrapper, {
                    children: menuItem,
                    selected,
                    icon,
                    label,
                    index,
                    isOpenVerticalMenu: false,
                  });
                }

                return menuItem;
              }),
            )}
          </div>
          <div className="jk-divider jk-pg-rl" style={{ boxSizing: 'border-box' }} />
          <div className="jk-row extend block">
            <div className="jk-col gap jk-pg-rl jk-pg-tb">
              <SettingsSection
                isOpen
                isMobile={true}
                helpOpen={helpOpen}
                setHelpOpen={setHelpOpen}
                popoverPlacement="top"
                moreApps={moreApps}
              />
            </div>
          </div>
        </>
      )}
    </div>
  );
};
