...
export const TableHeadFilterSelect = (props: TableHeadFilterSelectProps) => {
  const { onFilter, onReset, columnIndex, options, initialSelectedOptions, visible } = props;
  ...
  return (
    <div className="jk-col gap stretch jk-table-head-cell-filter-select jk-pg-sm">
      ...
      <div className="jk-row right gap">
        <Button size="tiny" type="secondary" onClick={onReset} disabled={!initialSelectedOptions.length}>
        ...
        <Button
          ...
          disabled={
            JSON.stringify(initialSelectedOptions.map(({ value }) => value)) ===
            JSON.stringify(selectedOptions.map(({ value }) => value))
          }
          ref={buttonRef}
        >
