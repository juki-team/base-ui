...
export const ViewContainerRows = <T,>(props: ViewContainerRowsProps<T>) => {
  ...
  useEffect(() => {
    ...
    const [visibleHeaders, totalMinUsedWidth, totalUsedWidth] = headers.reduce(
      ([visibleHeaders, totalMinUsedWidth, totalUsedWidth], { minWidth = minCellWidth, visible, width }) => [
    ...
    if (
      viewContainerWidth !== prevSizeWidth ||
      prevHeaders.current !== JSON.stringify(headersMinWidth(headers)) ||
      extra !== prevExtraWidth.current ||
      (visibleHeaders ? totalUsedWidth === 0 : false)
    ) {
