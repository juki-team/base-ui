  ...
  return (
    <div style={{}}>
      <FetcherLayer<ContentResponse<ProblemSummaryListResponseDTO>>
        url={
          jukiApiManager.API_V2.problem.getSummary({
            params: {
              key: problemKey,
            },
          }).url
        ...
        {({ data }) => {
          return (
            <div className="jk-col">
              ...
              <div>
                <T className="tt-se fw-bd">type</T>: {PROBLEM_TYPE[data.content.settings.type].label}
              </div>
              <div>
                <T className="tt-se fw-bd">scoring mode</T>: {PROBLEM_MODE[data.content.settings.scoringMode].label}
              </div>
            </div>
          );
        }}
