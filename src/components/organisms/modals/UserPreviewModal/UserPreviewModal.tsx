...
export function UserPreviewModal() {
  ...
  const [userPreviewNickname, userPreviewCompanyKey] = Array.isArray(userPreviewQuery)
    ? (userPreviewQuery as unknown as [string, string])
    : [userPreviewQuery as string, companyKey];
  ...
  return (
    <UserPreviewContentModal
      ...
      userHref={jukiAppRoutes.JUDGE().profiles.view({ nickname: userPreviewNickname, companyKey: userPreviewCompanyKey })}
    />
  );
}
