  ...
  return (
    <>
      <div className="content-log">
        <span className={classNames('text-log tx-xs tt-se', { 'cr-er': timeLimitExceded })}>
          {timeLimitExceded
            ? <><T>time limit exceeded</T> ({dataLogs[0]} <T>ms</T>)</>
            : <><T>used time</T>: {dataLogs[0]} <T>ms</T></>
          },
        </span>
        <span className={classNames('text-log tx-xs tt-se', { 'cr-er': memoryLimitExceded })}>
          {memoryLimitExceded
            ? <><T>memory limit exceeded</T> ({dataLogs[1]} <T>KB</T>)</>
            : <><T>used memory</T>: {dataLogs[1]} <T>KB</T></>
          },
        </span>
        <span className={classNames('text-log tx-xs tt-se', { 'cr-er': runtimeError })}>
