...
export function SplitModal<T extends ModalButtonLoaderEventType>(props: PropsWithChildren<SplitModalProps<T>>) {
  const { children, title, graphic, ...modalProps } = props;
  ...
  return (
    <Modal<T> {...modalProps}>
      <div className="split-modal jk-row stretch block">
        <div
          className="jk-side-secondary bc-at cr-at-it jk-br jk-col nowrap stretch jk-pg-lg"
          style={
            {
        ...
        >
          <div className="title" ref={titleSideSecondaryRef}>
            {renderReactNodeOrFunction(title)}
          </div>
