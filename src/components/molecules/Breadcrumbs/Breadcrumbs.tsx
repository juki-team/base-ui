...
export function Breadcrumbs({ breadcrumbs: initialBreadcrumbs, withoutHomeLink }: BreadcrumbsProps) {
  ...
  const onOverflow = useCallback(
    () => setReducedSize((prevState) => Math.min(prevState + 1, breadcrumbsLength)),
    [breadcrumbsLength],
  );
  ...
  return (
    <div className="jk-row nowrap left extend jk-breadcrumb" ref={refBreadcrumb} style={{ overflow: 'auto' }}>
      {/*<ArrowRightIcon />*/}
