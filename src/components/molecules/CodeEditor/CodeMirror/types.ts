...
export type DOMEventType = 'onMouseDown' | 'onDblClick' | 'onTouchStart' | 'onContextMenu' | 'onKeyDown' | 'onKeyPress'
  | 'onKeyUp' | 'onCut' | 'onCopy' | 'onPaste' | 'onDragStart' | 'onDragEnter' | 'onDragOver' | 'onDragLeave' | 'onDrop';

export type IDOMEventType = {
  [key in DOMEventType]?: (instance: CodeMirror.Editor, event: Event) => void;
}

export type EditorType = CodeMirror.Editor;

export type CodeMirrorType = typeof CodeMirror;

export type EditorFromTextAreaType = CodeMirror.EditorFromTextArea;

export interface IReactCodemirror extends IDOMEventType {
  /**
