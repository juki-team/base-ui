...
export function MultiSelectSearchable<T, U extends ReactNode, V extends ReactNode>(props: MultiSelectSearchableProps<T, U, V>) {
  ...
  return (
    <Popover
      popoverClassName="jk-select-options-content bc-sf-hi jk-br-ie elevation-1"
      ...
      content={
        ...
        >
          ...
          <div style={{ width: '100%', height: Math.ceil(256 / rowHeightOption) * rowHeightOption }}>
            <VirtualizedRowsFixed size={filteredOptions.length} rowHeight={rowHeightOption} renderRow={renderRow} />
          </div>
        </div>
      }
    >
        ...
        >
          <div className="jk-row left jk-multi-select-selected-options jk-pg-xsm">
            {selectedOptions.map((optionSelected) => (
              <div
                className={classNames('jk-row nowrap', { 'jk-tag bc-sf-hi': multiselect })}
